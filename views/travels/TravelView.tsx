...
const TravelView = () => {
  ...
  const [paxValues, setPaxValues] = useState<{ title: string; price: string }[]>(
  ...
  const handlePaxCountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPaxCount(count);
    setPaxValues(Array.from({ length: count }, (_, index) => paxValues[index] || { title: "", price: "" }));
  };
  ...
  const [formValues, setFormValues] = useState(
    Array.from({ length: sliderCount }, () => ({ direction: "", program: "" , photo :""}))
  );
