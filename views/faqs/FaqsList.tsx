  ...
  return (
    <>
      ...
      <div className="w-full border-b-[#e5e5e5] border-b h-20 bg-white flex items-center justify-between px-4 md:px-10 top-0 z-0">
        <span className="text-[#162C43] text-lg">Түгээмэл асуултуудийн жагсаалт</span>
        <Link
          href="/faqs/add"
      ...
      <div className="flex flex-col gap-4 w-full p-4 md:p-10">
        <div className="w-full flex items-center justify-between bg-white p-4 rounded-lg border border-[#e5e5e5]">
          <div className="flex flex-col text-[#162C43]">
            <span className="text-sm text-[#162C43] font-light">Нийт асуулт</span>
        ...
        <div className="w-full bg-white border p-4 rounded-lg overflow-x-auto">
          <table className="min-w-full text-[#162C43] rounded-lg w-full">
            <thead className="bg-[#FAFAFA] rounded-lg">
              <tr>
                <th className="w-[5%] text-center py-4 px-4 text-sm font-light">Д/д</th>
                <th className="w-[50%] text-start py-4 px-4 text-sm font-light">Асуулт</th>
                <th className="w-[20%] text-start py-4 px-4 text-sm font-light">Хариулт</th>
                <th className="w-[25%] text-start py-4 px-4 text-sm font-light">Үйлдэл</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={list._id}>
                  ...
                  <td>
                    <div className="flex gap-2 items-center">
                      <span className="text-xs line-clamp-1">{list?.question}</span>
                    </div>
                  </td>
                  <td className="text-xs line-clamp-1 text-start p-4 flex items-center">
                    <div className="flex gap-2 items-center">
                      <span className="text-xs line-clamp-1">{list?.answer}</span>
                    </div>
                  </td>
                  <td>
                    <div className="flex items-center gap-4">
                      <Link href={`/faqs/edit/${list._id}`}>
                        <Edit color="orange" size={20} className="cursor-pointer" />
                      </Link>
                      <Trash
                        color="red"
                        size={20}
